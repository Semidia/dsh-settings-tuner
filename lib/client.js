  ...
  factory: (require) => {
    ...
    var SECTIONS = [
      ...
      { ns: "llm-deepseek", label: "DeepSeek 官方", immediate: true, params: [
        ...
        { key: "thinking", label: "深度思考（仅官方）", desc: "只对官方 deepseek-official 生效，是否启用深度思考模式；推理强度请用上方「推理强度（全局统一）」", type: "select", default: "enabled", options: ["enabled", "disabled"] },
    ...
    function apply(ctx) {
      ...
      function SettingsTunerPage(props) {
        ...
        var _cc = React.useState({ found: false, config: {} }), compactCfg = _cc[0], setCompactCfg = _cc[1];
        var _ef = React.useState("off"), effort = _ef[0], setEffort = _ef[1];
        ...
        function loadAll() {
          ...
          api.settings.describe({}).then(function(res) {
            if (res && res.result && res.result.ok) {
              ...
              setValues(next);
              // 读取统一推理强度初始值（agent-default-model.reasoningEffort）
              var admNs = nss.find(function(n) { return n.ns === "agent-default-model"; });
              if (admNs && admNs.value && admNs.value.reasoningEffort !== void 0) {
                setEffort(String(admNs.value.reasoningEffort));
              }
          ...
          rpc.call(RPC_CHANNEL, "preset-compact/read", {}).then(function(res) {
            if (res && res.ok && res.value) setCompactCfg(res.value);
          }).catch(function() {});
        }
        ...
        function applyUnifiedEffort() {
          var val = effort;
          if (["off","low","high","max"].indexOf(val) < 0) { setStatus("错误：推理强度无效"); return; }
          setStatus("保存中…");
          Promise.all([
            api.settings.update({ ns: "agent-default-model", patch: { reasoningEffort: val } }),
            api.settings.update({ ns: "llm-deepseek", patch: { reasoningEffort: val } })
          ]).then(function() {
            loadAll();
            setStatus("✅ 已保存 · 全局默认推理强度 = " + val);
          }).catch(function(err) {
            setStatus("保存失败：" + String(err && err.message || err));
          });
        }
        ...
        function applyPresetCompact() {
          var cfg = compactCfg.config || {};
          if (typeof cfg.auto !== "boolean") { setStatus("错误：自动压缩必须是布尔值"); return; }
          if (typeof cfg.thresholdRatio !== "number" || !Number.isFinite(cfg.thresholdRatio) || cfg.thresholdRatio < 0.1 || cfg.thresholdRatio > 0.99) { setStatus("错误：压缩阈值必须在 0.1~0.99 之间"); return; }
          if (typeof cfg.retainRatio !== "number" || !Number.isFinite(cfg.retainRatio) || cfg.retainRatio < 0.01 || cfg.retainRatio > 0.5) { setStatus("错误：保留比例必须在 0.01~0.5 之间"); return; }
          if (typeof cfg.maxTokens !== "number" || !Number.isFinite(cfg.maxTokens) || cfg.maxTokens < 512 || cfg.maxTokens > 65536) { setStatus("错误：压缩摘要最大 Token 必须在 512~65536 之间"); return; }
          if (typeof cfg.maxOverflowRetries !== "number" || !Number.isFinite(cfg.maxOverflowRetries) || cfg.maxOverflowRetries < 0 || cfg.maxOverflowRetries > 10) { setStatus("错误：溢出重试次数必须在 0~10 之间"); return; }
          setStatus("写入中…");
          rpc.call(RPC_CHANNEL, "preset-compact/write", { config: cfg }).then(function(res) {
            if (res && res.ok) setStatus(res.message || "已写入 ✓ 下次会话生效");
            else setStatus("写入失败：" + (res && res.error && res.error.message || "未知错误"));
          }).catch(function(err) {
            setStatus("写入失败：" + String(err && err.message || err));
          });
        ...
        var showUnifiedEffort = !q || q.indexOf("推理") >= 0 || q.indexOf("思考") >= 0 || q.indexOf("强度") >= 0 || q.indexOf("effort") >= 0;
        ...
        return React.createElement("div", null,
          ...
          showUnifiedEffort ? React.createElement("div", { className: "dst-section" },
            React.createElement("div", { className: "dst-section-head" },
              React.createElement("span", { className: "dst-section-title" }, "推理强度（全局统一）"),
            ...
            React.createElement("div", { className: "dst-rows" },
              React.createElement("div", { className: "dst-row" },
                React.createElement("div", { className: "dst-info" },
                  React.createElement("span", { className: "dst-label" }, "全局默认推理强度"),
                  React.createElement("span", { className: "dst-desc" }, "唯一入口：同时写入「默认模型」与「DeepSeek 官方」。新会话以此为准，切换模型弹窗默认显示此值。"),
                  React.createElement("span", { className: "dst-desc" }, "两边自动同步：弹窗里手选的会写回此默认，此处改的会成为弹窗默认——无需记优先级，以这里为准即可。"),
                  React.createElement("span", { className: "dst-desc" }, "当前值：" + effort)
                ),
                React.createElement("select", { className: "dst-select", value: String(effort), onChange: function(e) { setEffort(e.target.value); } },
                  ["off","low","high","max"].map(function(o) { return React.createElement("option", { key: o, value: o }, o); })
                ),
                React.createElement("button", { className: "dst-btn primary", onClick: function() { applyUnifiedEffort(); } }, "应用")
              )
            )
          ) : null,
          ...
          compactCfg.found ? React.createElement("div", { className: "dst-section" },
            React.createElement("div", { className: "dst-section-head" },
              React.createElement("span", { className: "dst-section-title" }, "压缩参数（预设平面）"),
              React.createElement("span", { className: "dst-badge restart" }, "下次会话生效")
            ),
            React.createElement("div", { className: "dst-rows" },
              [
                { key: "auto", label: "自动压缩", desc: "上下文达到阈值时自动触发压缩", type: "boolean" },
                { key: "thresholdRatio", label: "压缩阈值", desc: "触发压缩的上下文占用比例（0.1~0.99）", type: "number", min: 0.1, max: 0.99, step: 0.05 },
                { key: "retainRatio", label: "保留比例", desc: "压缩后保留的最近上下文比例（0.01~0.5）", type: "number", min: 0.01, max: 0.5, step: 0.01 },
                { key: "maxTokens", label: "压缩摘要最大 Token", desc: "压缩摘要单次生成最大 token 数", type: "number", min: 512, max: 65536, step: 512 },
                { key: "maxOverflowRetries", label: "溢出重试次数", desc: "上下文溢出时的重试次数", type: "number", min: 0, max: 10, step: 1 }
              ].map(function(p) {
                var val = compactCfg.config[p.key];
                var isBool = p.type === "boolean";
                var invalid = isBool ? false : (typeof val !== "number" || !Number.isFinite(val) || val < p.min || val > p.max);
                var changed = val !== undefined;
                return React.createElement("div", { key: p.key, className: "dst-row" },
                  React.createElement("div", { className: "dst-info" },
                    React.createElement("span", { className: "dst-label" + (changed ? " override" : "") }, p.label + (changed ? " ●" : "")),
                    React.createElement("span", { className: "dst-desc" }, p.desc)
                  ),
                  isBool
                    ? React.createElement("input", { type: "checkbox", className: "dst-input", checked: val === true, onChange: function(e) { var next = Object.assign({}, compactCfg, { config: Object.assign({}, compactCfg.config) }); next.config[p.key] = e.target.checked; setCompactCfg(next); } })
                    : React.createElement("input", { className: "dst-input" + (invalid ? " invalid" : ""), type: "number", value: val === undefined ? "" : val, min: p.min, max: p.max, step: p.step, onChange: function(e, pk = p.key) { var next = Object.assign({}, compactCfg, { config: Object.assign({}, compactCfg.config) }); next.config[pk] = Number(e.target.value); setCompactCfg(next); } }),
                  React.createElement("button", { className: "dst-btn primary", onClick: function() { applyPresetCompact(); } }, "应用")
                );
              }),
              React.createElement("div", { className: "dst-row" },
                React.createElement("div", { className: "dst-info" },
                  React.createElement("span", { className: "dst-desc" }, "写入 preset 平面（" + (compactCfg.preset || "?") + "），非顶层组合树；改后新会话生效。" + (compactCfg.file ? " 文件: " + compactCfg.file : ""))
                ),
                React.createElement("button", { className: "dst-btn", onClick: function() { rpc.call(RPC_CHANNEL, "preset-compact/read", {}).then(function(res) { if (res && res.ok && res.value) setCompactCfg(res.value); setStatus("已刷新当前值"); }).catch(function() {}); } }, "刷新")
              )
            )
          ) : null,
